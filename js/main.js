window.translations = {
    ...
    'vi': {
        ...
        'featured': 'Nổi bật',
        'view_all': 'Xem tất cả',
        'footer_desc': 'Nền tảng đánh giá phim tuyệt đỉnh với công nghệ hiện đại.',
        'genre_action': 'Hành động',
        'genre_scifi': 'Viễn tưởng',
        'genre_drama': 'Chính kịch',
        'director': 'Đạo diễn',
        'original_creator': 'Nguyên tác/Tác giả',
        'episodes': 'Danh sách tập',
        'links': 'Liên kết',
        'subscribe': 'Nhận tin',
        'join': 'Tham gia',
        'add_watchlist': 'Thêm vào DS',
        'added': 'Đã thêm',
        'genre_anime': 'Anime',
        'genre_horror': 'Kinh dị',
        'genre_comedy': 'Hài kịch',
...
$(document).ready(function () {
    ...
    function renderMovieCard(movie) {
        ...
        return `
            <div class="col">
                <div class="card h-100 movie-card bg-transparent border-0">
    ...
    if ($('#movies-grid').length > 0) {
        ...
        // Render mặc định
        renderMoviesSearch('All', '');

        // ---- YÊU CẦU JQUERY 2: Filter Form (Modern Menu & Search) ----
        $('.genre-link').click(function(e) {
            ...
            renderMoviesSearch(selectedGenre, currentSearch);
        });

        $('#sideSearch').on('input', function() {
            currentSearch = $(this).val().toLowerCase();
            let selectedGenre = $('.genre-link.active').data('genre') || 'All';
            renderMoviesSearch(selectedGenre, currentSearch);
        });
    }
        ...
        if (movie) {
            ...
            if (isSeries && hasSeasons) {
                ...
                function renderSeasonUI() {
                    ...
                    if (showSwitcher) {
                        seasonTabsHtml += `
                            <div class="liquid-glass rounded-pill p-1 px-4 d-inline-flex align-items-center gap-4 mb-4 border border-white border-opacity-10 shadow-lg">
                                ...
                                <a href="top-rated.html" class="text-decoration-none text-white-50 small hover-white">Top-rated</a>
                            </div>
                        `;
                    } else {
                        const singleTitle = activeTab === 'seasons' ? t['seasons'] : t['movies'];
                        seasonTabsHtml += `
                            <div class="liquid-glass rounded-4 p-3 d-flex align-items-center justify-content-between mb-4 border border-white border-opacity-10 shadow-lg">
                                ...
                                <a href="top-rated.html" class="text-decoration-none text-white-50 small hover-white">Top-rated <i class="bi bi-arrow-right ms-1"></i></a>
                            </div>
                        `;
                    }
                                ...
                                <div class="episode-rich-card liquid-glass p-3 rounded-4 d-flex gap-4 mb-3" style="cursor: pointer;" data-id="${ep.id}" data-embed="${ep.embed}" data-season="${sId}">
                                    ...
                                    <div class="ep-content flex-grow-1">
                                        ...
                                        <div class="d-flex align-items-center gap-3">
                                            ...
                                            <button class="btn btn-sm btn-link text-primary text-decoration-none p-0 small"><i class="bi bi-star"></i> Rate</button>
